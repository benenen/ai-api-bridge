...
│   ├── store.rs             # SQLite 数据访问层（providers/routes/status CRUD）
...
定义 `Config`、`Provider`、`Route`、`RouteTarget`、`WireName` 等核心结构体：

- **Config**: `listen`、`database`、`default_provider`、`auth_token`、`providers`（HashMap）、`routes`（Vec）
- **Provider**: `wire`（协议）、`base_url`、`api_key`、`model_prefix`、`max_tokens_field`、`extra_headers`、`probe_script`（Lua 脚本路径）、`probe_enabled`、`probe_interval_secs`、`quota_min`
...
| `GET /v1/providers` | — | 提供商状态（watcher 数据） |
| `GET /health` | — | 健康检查 |
| `/` `/admin` | HTML | 管理控制台 |
| `/admin/api/providers` | REST | 提供商 CRUD |
| `/admin/api/routes` | REST | 路由 CRUD |
...
| 表 | 说明 |
|---|---|
| `providers` | Provider 配置（name PK, wire, base_url, api_key, model_prefix, max_tokens_field, extra_headers, probe_script, probe_enabled, probe_interval_secs, quota_min） |
...
- `load_into_config()` — 从 DB 加载到内存配置
- `insert_provider/update_provider/delete_provider` — 管理 CRUD
- `write_status()` — watcher 写入状态（upsert）
- `load_statuses()` — 启动时恢复持久化状态
...
- `GET /admin/api/routes` — 列出所有路由
- `POST /admin/api/routes` — 创建路由
- `PUT /admin/api/routes/:alias` — 更新路由
- `DELETE /admin/api/routes/:alias` — 删除路由

所有写入操作完成后自动调用 `reload_from_db()` 使变更热生效（无需重启）。鉴权复用 bridge 的 `auth_token`。
